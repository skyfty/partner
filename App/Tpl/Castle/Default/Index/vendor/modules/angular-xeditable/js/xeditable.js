...
angular.module('xeditable').directive('editableDateRange', ['editableDirectiveFactory',
    function(editableDirectiveFactory) {
        return editableDirectiveFactory({
            directiveName: 'editableDateRange',
            inputTpl: '<div class="input-daterange input-group" style="display: inline-table"></div>',
            render: function() {
                var self = this;
                this.parent.render.call(this);

                var start="",end="";
                var date = self.scope.$data;
                if (angular.isDefined(date) && angular.isArray(date) && date.length == 2) {
                    if (date[0]) {
                        start = " value='" + date[0] + "'";
                    }
                    if (date[1]) {
                ...
                var html = '<input type="text" '+start+' class="editable-input form-control ng-pristine ng-valid ng-touched range-start"/>';
                html += '<span class="input-group-addon">至</span><input type="text" '+end+' class="editable-input form-control ng-pristine ng-valid ng-touched  range-end"/>';
                this.inputEl.html(html);
            ...
            onsaveing:function(){
                var self = this;
                var start_el = this.inputEl.find(".range-start");
                var start = start_el.val();
                var end_el = this.inputEl.find(".range-end");
                var end = end_el.val();
                self.scope.$data = (start && end) ? [start,end]:"";
            }
        });
    }]);

angular.module('xeditable').directive('editableAddress', ['editableDirectiveFactory','$parse',
    function(editableDirectiveFactory,$parse) {
        return editableDirectiveFactory({
            ...
            render: function() {
                ...
                var data = self.scope.$data;

                var clas = "editable-input form-control ng-pristine ng-valid ng-touched";
                var html =
                    '<select class="'+clas+' state" onchange="state_change(this);"></select>'+
                    '<select class="'+clas+' city" onchange="city_change(this);"></select>'+
                    '<select class="'+clas+' area"></select>'+
                    '<input  class="'+clas+' street" type="text" value="'+(data&&data[3]?data[3]:"")+'">';
                self.inputEl.html(html);
                init_area(self.inputEl, data||[0,0,0,""]);
            },

            onsaveing:function(){
                var self = this;
                var state_el = self.inputEl.find(".state");
                var state = state_el.val();
                var city_el = self.inputEl.find(".city");
                var city = city_el.val();
                var area_el = self.inputEl.find(".area");
                var area = area_el.val();
                var street = self.inputEl.find(".street").val();
                self.scope.$data = [state,city,area,street];
            }
        });
    }
]);

angular.module('xeditable').directive('editableSelectBox', ['editableDirectiveFactory','$parse',
    function(editableDirectiveFactory,$parse) {
        return editableDirectiveFactory({
            directiveName: 'editableSelectBox',
            inputTpl: '<select  class="editable-input form-control ng-pristine ng-valid ng-touched "></select>',
            render: function() {
                var self = this;
                this.parent.render.call(this);
                var fieldOptions = $parse(this.attrs.fieldOptions)(self.scope);

                var options = '';
                if (fieldOptions.multiple != "multiple") {
                    options = '<option value="">请选择...</option>';
                }else {
                    this.inputEl.addClass("selectpicker");
                    this.inputEl.attr("multiple","multiple");
                }
                angular.forEach(fieldOptions.data, function(d){
                    options += '<option value="'+ d.value+'">'+ d.text+'</option>';
                });
                this.inputEl.html(options);

                if (fieldOptions.multiple == "multiple") {
                    this.inputEl.selectpicker({noneSelectedText:"选择一些选项..."}).selectpicker('val', self.scope.$data);
...
angular.module('xeditable').directive('editableConditionField', ['editableDirectiveFactory','$parse',
    function(editableDirectiveFactory,$parse) {
        return editableDirectiveFactory({
            directiveName: 'editableConditionField',
            inputTpl: '<span></span>',
            render: function() {
                var self = this;
                self.parent.render.call(this);
                var html = $('<ui-condition style="width: 100%" class="input-group" cas-model="'+self.name+'" ng-field="'+self.attrs.fieldOptions+'"></ui-condition>');
                if (angular.isDefined(self.attrs.popover)) {
                    html.attr("popover",self.attrs.popover);
                }
                this.inputEl.html(html);
            },
            ...
            onsaveing:function(){
                var self = this;
                var data = $parse(self.name)(self.scope);
                $parse(self.name).assign(self.scope,self.scope.$data = {value:data.value,condition:data.condition});
            }
        });
    }
]);
...
angular.module('xeditable').directive('editableSelectAccountTypeBox', ['editableDirectiveFactory','$parse',
    function(editableDirectiveFactory,$parse) {
        return editableDirectiveFactory({
            directiveName: 'editableSelectAccountTypeBox',
            inputTpl: '<select class="editable-input form-control ng-pristine ng-valid ng-touched"></select>',
            render: function() {
                var self = this;
                self.parent.render.call(self);
                var fieldOptions = $parse(self.attrs.fieldOptions)(self.scope);

                var options =   '<option value="">请选择...</option>';
                angular.forEach(fieldOptions.data, function(d){
                    options += '<option value="'+ d.type_id+'">'+ d.name+'</option>';
                });

                self.inputEl.html(options);
            },
            onaftersave:function(){
                var self = this;
                var fieldOptions = $parse(self.attrs.fieldOptions)(self.scope);
                var account_type = fieldOptions.data[self.scope.$data];
                $parse(self.attrs.editableSelectAccountTypeBox).assign(self.scope,account_type.type_id);
            }
        });
    }
]);
...
angular.module('xeditable').directive('editableSelectPcatBox', ['editableDirectiveFactory','$parse',
    function(editableDirectiveFactory,$parse) {
        return editableDirectiveFactory({
            directiveName: 'editableSelectPcatBox',
            inputTpl: '<select class="editable-input form-control ng-pristine ng-valid ng-touched"></select>',
            render: function() {
                var self = this;
                this.parent.render.call(this);
                ...
                var options =   '<option value="">请选择...</option>';
                var catoptions = [];
                if (angular.isDefined(self.attrs.onfilter)) {
                    catoptions = self.scope.$eval(self.attrs.onfilter);
                } else {
                    catoptions = fieldOptions.data;
                }
                angular.forEach(catoptions, function(d){
                    options += '<option value="'+ d.category_id+'">'+ d.name+'</option>';
                });
            ...
            onaftersave:function(){
                var self = this;
                var fieldOptions = $parse(this.attrs.fieldOptions)(self.scope);
                var product_category = fieldOptions.data;
                var category_name_mode = self.attrs.editableSelectPcatBox.replace("category_id", "name");
                var category = product_category[self.scope.$data];
                $parse(self.attrs.editableSelectPcatBox).assign(self.scope,category.category_id);
                $parse(category_name_mode).assign(self.scope,category.name);
            }
        });
    }
]);

angular.module('xeditable').directive('editableModelId', ['modelinfo', 'editableDirectiveFactory','$parse',
    function(modelinfo, editableDirectiveFactory,$parse) {
        return editableDirectiveFactory({
            directiveName: 'editableModelId',
            inputTpl: '<input readonly="readonly" class="editable-input form-control ng-pristine ng-valid ng-touched"/>',
            render: function() {
                var self = this;
                this.parent.render.call(this);
                if (this.scope.$data) {
                    this.scope.$data = this.scope.$data.name;
                }
                self.scope.respond_model = function(data){
                    var modelId = self.attrs.editableModelId;
                    $parse(modelId).assign(self.scope, data);
                    if (angular.isDefined(fieldOptions.onSelected)) {
                        self.scope.$eval(fieldOptions.onSelected);
                    }
                };

                var param = [];
                var fieldOptions = $parse(self.attrs.fieldOptions)(self.scope);
                if (angular.isDefined(fieldOptions.onActive)) {
                    param = self.scope.$eval(fieldOptions.onActive);
                }
                modelinfo['select_' + self.attrs.modelName](param).then(self.scope.respond_model);
            }
        });
    }
]);
...
angular.module('xeditable').directive('editableChannelRoleModelBox', ['editableDirectiveFactory','$parse',
    function(editableDirectiveFactory,$parse) {
        return editableDirectiveFactory({
            directiveName: 'editableChannelRoleModelBox',
            inputTpl: '<select  class="editable-input form-control ng-pristine ng-valid ng-touched"></select>',
            render: function() {
                var self = this;
                this.parent.render.call(this);
                ...
                self.inputEl.html(options);
                self.scope.$data = self.scope.$data.channel_id;
            },
            onsaveing:function(){
                var self = this;
                var fieldOptions = $parse(self.attrs.fieldOptions)(self.scope);
                self.scope.$data = fieldOptions.data[self.scope.$data];

                var channelRoleIdModel = self.attrs.editableChannelRoleModelBox.replace("channel_role_model","channel_role_id");
                if (self.scope.$data.model == "渠道库") {
                    $parse(channelRoleIdModel).assign(self.scope, angular.copy(self.scope.$data));
                } else {
                    $parse(channelRoleIdModel).assign(self.scope, null);
                }
            }
        });
    }
...
angular.module('xeditable').directive('editableChannelRoleIdBox', ['editableDirectiveFactory','$parse','modelinfo',
    function(editableDirectiveFactory,$parse,modelinfo) {
        return editableDirectiveFactory({
            directiveName: 'editableChannelRoleIdBox',
            inputTpl: '<input  readonly="readonly" class="editable-input form-control ng-pristine ng-valid ng-touched"/>',
            ...
            render: function() {
                ...
                var channelModel = self.attrs.editableChannelRoleIdBox;
                var channel = $parse(channelModel.replace("channel_role_id","channel_role_model"))(self.scope);

                if (angular.isDefined(channel) && channel) {
                    modelinfo.select_channel_related(channel).then(function(d){
                        $parse(channelModel).assign(self.scope, d);
                    });
                }
            }
        });
...
angular.module('xeditable').directive('editableBranchBox', ['editableDirectiveFactory','$parse','modelinfo',
    function(editableDirectiveFactory,$parse,modelinfo) {
        return editableDirectiveFactory({
            directiveName: 'editableBranchBox',
            inputTpl: '<input  readonly="readonly" class="editable-input form-control ng-pristine ng-valid ng-touched"/>',
            ...
            render: function() {
                ...
                var branchModel = self.attrs.editableBranchBox;
                modelinfo.select_branch({}).then(function(d){
                    $parse(branchModel).assign(self.scope, d);
                });
            }
        });
    }
    ...
    self.init = function(single) {
      ...
      */
      ...
      if ($attrs.onbeforesave) {
        ...
        if ($attrs.onsaveing) {
            self.onsaveing = function() {
                return self.catchError($parse($attrs.onsaveing)($scope));
            };
        }
      ...
      link: function(scope, elem, attrs, ctrl) {
        ...
        */
        ...
        var is_disabled = function() {
            if (angular.isDefined(attrs.editDisabled)) {
                return scope.$eval(attrs.editDisabled);
            }

            if (angular.isDefined(attrs.fieldOptions)) {
                var fieldOptions = $parse(attrs.fieldOptions)(scope);
                if (fieldOptions && angular.isDefined(fieldOptions.disabled)) {
                    return fieldOptions.disabled;
                }
            }
          return editableOptions.isDisabled;
        };
          ...
          post: function(scope, elem, attrs, ctrl) {
            ...
            if(!attrs.ngSubmit && !attrs.submit) {
              ...
              if(attrs.onbeforesave) {
                ...
                if(attrs.onsaveing) {
                    eForm.$onsaveing = function() {
                        return $parse(attrs.onsaveing)(scope, {$data: eForm.$data});
                    };
                }
    ...
    'bs3': {
      ...
      postrender: function() {
        ...
        switch(this.directiveName) {
          case 'editableText':
          case 'editableSelect':
          case 'editableSelectBox':
          case 'editableChannelRoleIdBox':
