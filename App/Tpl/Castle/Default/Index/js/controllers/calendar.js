...
app.controller('calendarController', ['$scope','$state', function($scope, $state) {
    ...
    $.ajax({'dataType': 'json','url': 'product/listevent','data':{'product_id':$scope.casModel.product_id},
        'success': function (events) {
            if (events && events.data) {
                var events = events.data;
                for (var i in events) {
                    var e = {};
                    e.start = new Date(events[i].start.replace(/-/g, "/"));
                    e.end = new Date(events[i].end.replace(/-/g, "/"));
                    e.info = events[i].description;
                    e.title = "lsjdflskjdf";
                    e.location ='New York';
                    var bd = moment(events[i].start).get("date"), ed = moment(events[i].end).get("date");
                    var bm = moment(events[i].start).get("month"), em = moment(events[i].end).get("month");
                    //var cd = moment(date).get("date");
                    //var cm = moment(date).get("month");
                    //var md = ((bm == cm && bd == cd) || (em == cm && ed == cd));

                    var wid = events[i].workstate_id;
                    var cls = {
                        disabled: wid=="请假"||wid=="公司培训"||wid=="司外订单"||wid == "上岗",
                    };
                    if (wid == "请假") {
                        e.className = ['b-l b-2x b-info'];
                    } else if (wid == "公司培训") {
                        e.className = ['b-l b-2x b-info'];
                    }else if(wid == "司外订单"){
                        e.className = ['b-l b-2x b-info'];
                ...
                $scope.events.push({title:'All Day Event', start: new Date(y, m, 1), className: ['b-l b-2x b-info'], location:'New York', info:'This a all day event that will start from 9:00 am to 9:00 pm, have fun!'});
                $scope.eventSources = [$scope.events];
                $scope.$apply();
            }
        },
