...
class ProductEvaluateAction extends CastleAction {
    ...
    public function get_module_view() {
        return D($_GET['forces']['et'] == "market" ? "MarketProductEvaluateView":"ProductEvaluateView");
    }

    public function show_list($where = array(), &$params = array()) {
        if (isset($where['product_evaluate.et'])) {
            if ($where['product_evaluate.et'][1] == "market") {
                $where['salary_settle_time'] = array("neq", "");
            }
            unset($where['product_evaluate.et']);
        }
    ...
    public function replenish_list($list, $export = false) {
        $field_array = array();
        $field_array[] = array("field"=>"update_time", "form_type"=>"datetime","in_index"=>1, "name"=>"创建时间");
        $field_array[] = array("field"=>"owner_role_id", "model"=>"log","form_type"=>"user","in_index"=>1, "name"=>"所有者");
        $field_array[] = array("field"=>"market_owner_role_id", "model"=>"log","form_type"=>"user","in_index"=>1, "name"=>"所有者");
        foreach($list as $k=>$v) {
            $list[$k] = fill_fields($field_array, $this->getmodulename(), $v);
        }
        return $list;
    }
