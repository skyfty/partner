...
class LogAction extends Action{
	...
	public function add(){
        if($_POST['r']){
            $r = $_POST['r'];
            $_POST['subject'] = $_POST['subject'] ? $_POST['subject']:"日志记录";
            $module = $_POST['module'];
            $model_id = $_POST['id'];
            $m_r = M($r);
            $m_log = M('Log');
			...
			$musr = M("user")->where("role_id=".$m_log->role_id)->find();
			if ($musr) {
				$m_log->role_id_keyword = implode(chr(10), array($musr['name'],$musr['idcode'],$musr['telephone']));
			}
			$m_log->create_date =$m_log->update_date =  time();
            if(!($log_id = $m_log->add())){
                $this->ajaxReturn(array("error"=>"日志添加失败"));
            }
            ...
            $data['assort'] = $_POST['assort'];
            $m_module = M($module)->where(array($m_id=>$model_id))->find();
            if ($m_module) {
                $data[$module . '_idcode'] = $m_module['idcode'];
            }
            if ($module == "commiss" && isset($_POST['logpubshow'])) {
                $data['logpubshow'] = ($_POST['logpubshow'] == "on"?1:0);
            }
            if($m_r->add($data)){
                if ($module == "commiss") {
                    M("commiss")->where("commiss_id=".$model_id)->setField("last_log_time", time());
                }
	...
	public function index(){
        ...
        if (!empty($_REQUEST['assort'])){
            $params['assort'] = 'assort=' . trim($_REQUEST['assort']);
		...
		if (!empty($_REQUEST['r']) && !empty($_REQUEST['module'])) {
            ...
            if (!empty($_REQUEST['product_id'])) {
                $params['product_id'] = 'product_id=' . trim($_REQUEST['product_id']);
            }
            if (!empty($_REQUEST['customer_id'])) {
                $params['customer_id'] = 'customer_id=' . trim($_REQUEST['customer_id']);
            }
		...
		foreach($list as $key=>$value){
			...
			$list[$key]['pre_delete'] = ($value['role_id'] == session("role_id") || session('?admin'));
