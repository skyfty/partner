<?php
class AccountAction extends CastleAction{
    ...
    public function format_account_info($v, $export = false) {
        $v['creator_role_id'] = D('StaffRoleView')->where('user.role_id = %d', $v['creator_role_id'])->find();
        if ($v['infow_account_id']) {
            if ($v['inflow_model'] == "inernal") {
                $infow['name'] = session('league_name');
        ...
        if ($v['related_model'] == 'trade') {
            $v['related'] = D("TradeAccountView")->where(array('account_id'=>$v['account_id']))->find();
            $owner_role = D('StaffRoleView')->where('user.role_id = %d', $v['related']['owner_role_id'])->find();
            if ($owner_role) {
                $v['related']['owner_role_id'] = $owner_role;
                $v['related']['corre'] = D(ucfirst($v['trade']['corre'])."TradeAccountView")->where(array('account_id'=>$v['account_id']))->find();
            }
        } else if ($v['related_model'] == 'market') {
            if ($v['account_type'] != "market") {
                $v['related'] = D("MarketRelatedAccountView")->where(array('account_id'=>$v['account_id']))->find();
            } else {
                $v['related'] = D("MarketAccountView")->where(array('account_id'=>$v['account_id']))->find();
            }
            $v['related']['owner_role'] = D('StaffRoleView')->where('user.role_id = %d', $v['related']['owner_role_id'])->find();
            $v['related']['verify_role'] = D('StaffRoleView')->where('user.role_id = %d', $v['related']['verify_role_id'])->find();
            $v['related']['corre'] = D("MarketAccountView")->where(array('account_id'=>$v['account_id']))->find();

        } else if ($v['related_model'] == 'cultivate') {
            if ($v['account_type'] != "cultivate") {
                $v['related'] = D("CultivateRelatedAccountView")->where(array('account_id'=>$v['account_id']))->find();
            } else {
                $v['related'] = D("CultivateAccountView")->where(array('account_id'=>$v['account_id']))->find();
            }
            $v['related']['owner_role'] = D('StaffRoleView')->where('user.role_id = %d', $v['related']['owner_role_id'])->find();
            $v['related']['verify_role'] = D('StaffRoleView')->where('user.role_id = %d', $v['related']['verify_role_id'])->find();
            $v['related']['corre'] = D(ucfirst($v['cultivate']['corre'])."CultivateAccountView")->where(array('account_id'=>$v['account_id']))->find();

        } else if ($v['related'] && $v['related_id']) {
            $related = $v['related'];
            $v['related'] = D(ucfirst($v['related'])."View")->where(array($related.'_id'=>$v['related_id']))->find();
            $v['related']['owner_role_id'] = D('StaffRoleView')->where('user.role_id = %d', $v['related']['owner_role_id'])->find();
            $v['related'] = D(ucfirst($v['related']['corre']).ucfirst($related)."View")->where(array($related.'_id'=>$v['related_id']))->find();

        } else if ($v['related_id']) {
            $ra = M("account")->where(array('account_id'=>$v['related_id']))->find();
            if ($ra) {
                if ($ra['account_type'] == 'internal') {
                    $v['related_id'] = session('league_name');
                } else {
                    $v['related_id'] = M($ra['account_type'])->cache(true)->where(array($ra['account_type'].'_id'=>$ra['clause_additive']))->find();
                }
            }
        } else if ($v['related']) {
    ...
    public function show_list($where = array(), $params = array()) {
        ...
        if ($_GET['clause_additive']) {
            $where['account.clause_additive'] =   $_GET['clause_additive'];
        }
    ...
    public function get_module_view() {
        return D(ucfirst($this->type).'AccountView');
    }

    function perfect_list_item($value, $export = false) {
        $this->module = strtolower(MODULE_NAME);
        return parent::perfect_list_item($this->format_account_info($value));
    }
