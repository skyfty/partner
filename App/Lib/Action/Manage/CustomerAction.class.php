...
class CustomerAction extends BaseAction {
    ...
    public function show_list($where = array(), $params = array()) {
        ...
        if ($branch_id != "") {
            if ($branch_id == 0) {
                $where['customer.branch_id'] = array("in", array("0", ""));
            } else {
        ...
        self::show_list_index_html($where, $params, "客户表");
    }
    ...
    public function listDialog(){
        ...
        $m_model_name = $_GET['model']? $_GET['model']."View":"CustomerView";
    ...
    public function parse_dialog_where() {
        ...
        if(!empty($business_id)){
            $customer_ids = M('business')->where('business_id = %d',$business_id)->getField('customer_id', true);
