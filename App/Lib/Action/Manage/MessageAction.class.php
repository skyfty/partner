...
class MessageAction extends Action{
	...
	public function tips(){
		//导航顶部数字提示
		$new_num['message'] = M('message')->where(array('to_role_id' => session('role_id'),'read_time' => 0,'status'=>array('neq', 1)))->count();

        $m_task = M('Task');
		$new_num['task'] = $m_task->where('owner_role_id like "%s" and isclose = 0 and status <> "'.L('COMPLETE').'"', '%'.session('role_id').'%',time())->count();

		$last_read_time_js = M('user')->where('role_id = %d', session('role_id'))->getField('last_read_time');
		if(!empty($last_read_time_js)){
			$last_read_time = json_decode($last_read_time_js, true);
		}
		//任务
        $where = array(
            'owner_role_id'=>array("like", '%'.session('role_id').'%'),
            'create_date'=>array("gt", $last_read_time['task']),
            'end_time'=>array("gt", time()),
        );
		$new_num['task_count'] = $m_task->where($where)->count();
