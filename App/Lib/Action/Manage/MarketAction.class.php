...
class MarketAction extends BaseAction{
	...
	public function _initialize(){
		...
		$action = array(
			'permission'=>array(
                ...
                'account_list',
                'submit_settlement',
                'revoke_submit_settlement',
                'retreat_submit_settlement',
                'settlement',
                'payment_status',
                'payment_verify',
                'native_payment_verify',
                'marketadd',
                'quick_create_customer_dialog',
                'getmarketproduct',
                'update_urge_price_by_branch',
                'update_urge_price_by_category',
                'ns',
                'pv',
                'make_owner_role_id_list_header'
            ),
			'allow'=>array(
                "cancel_submit",
                "mission",
        ...
        if (ACTION_NAME == "index") {
            if (($_REQUEST['act'])) {
                if ($_REQUEST['act'] == "advsearch") {
                    ...
                    if (vali_permission("market", ACTION_NAME, $_REQUEST['act'])) {
                        if(isAjaxRequest()){
                            echo '<div class="alert alert-error">您没有此权利！</div>';die;
                        }else{
                            alert('error', '您没有此权利!', empty($_SERVER['HTTP_REFERER']) ? U('index/index') : $_SERVER['HTTP_REFERER']);
                        }
                    }
                }
            }
            ...
            if (!$assort) {
                foreach(array("xs", "cb") as $v) {
                    if (vali_permission("market", ACTION_NAME, $v)) {
                        $assort = $v;
                        break;
                    }
                }
            }
            $_REQUEST['t'] = $assort?$assort:"xs";

        } else if (ACTION_NAME == "view") {
            if ($_REQUEST['assort']) {
                if (vali_permission("market", ACTION_NAME, $_REQUEST['assort'])) {
                    $action['allow'][] = "index";
                } else {
            ...
            if (vali_permission("market", "product_edit")) {
                $action['allow'][] = "evaluate";
            } else {
                alert('error', '您没有此权利!', empty($_SERVER['HTTP_REFERER']) ? U('index/index') : $_SERVER['HTTP_REFERER']);
            }
        }
	...
	**/
    ...
    public function getmarket() {
        $id = $this->_request("id");
        $market = D('MarketView')->where(array("market.market_id"=>$id))->find();

        $this->ajaxReturn($market,"", $market != false ? 1 : 0);
    }
    ...
    public function format_index_fields($field_array) {
        ...
        foreach($field_array as $k=>$v) {
            if ($v['field'] == "agency_scale"){
                $sum_cnt = $m_module->where($this->list_where)->sum('product_agency_scale');
                $v['link_title'] = $v['name']."合计：" . number_format($sum_cnt, 2)."元";
            }elseif ($v['form_type'] == "floatnumber") {
    ...
    public function field_where($field, $search, $condition) {
        if ('service_product' == $field || $field == "market.service_product") {
            $inwhere = array();
            $inwhere["product.name"] = like_filed_where($search, $condition);
            $inwhere["product.idcode"] = $search;
            $inwhere["product.telephone"] = $search;
            $inwhere["product.league_id"] = session("league_id");

            $inwhere['_logic'] = 'OR';
            $insurance_query = D("MarketProductView")->where($inwhere)->getField("market_id", true);
            $where['market.market_id'] = array("in", $insurance_query);
        } else {
    ...
    public function show_list($where = array(), $params = array()) {
        $this->assort = isset($_GET['assort']) ? trim($_GET['assort']) : 'xs';
        $params[] = "assort=" . $this->assort;
        ...
        if ($branch_id != "") {
            $where['market.branch_id'] = $branch_id;
        ...
        $bysd = isset($_GET['bysd']) ? trim($_GET['bysd']) : '';
        switch ($bysd) {
            case 'today' :
                $where['settle_date'] =  array('gt',strtotime(date('Y-m-d', time())));
                break;

            case 'fmonth' :
                $where['settle_date'] =  array('between',getLastMonthDays(time()));
                break;

            case 'month' :
                $where['settle_date'] = array('gt',strtotime(date('Y-m-01', time())));
                break;
        }
        if (!empty($_GET['bysd'])) {
            $params[] = "bysd=".trim($_GET['bysd']);
        }

        if (isset($_GET['asids'])) {
            $this->asids = trim($_GET['asids']);
            $where['market.status_id'] = array("in", $this->asids);
            $params[] = "asids=" . $this->asids;
        } else {
            $where['market.status_id'] = array("neq", '0');
        }


        $bc = F("business");
        if (isset($_GET['asid'])) {
            $this->asids = $asid = trim($_GET['asid']);
            if ($asid == 4) {
                $where['market.status_id'] = 1;
                $where['_string'] = 'market.start_time!="" and TO_DAYS(from_unixtime(market.start_time)) - TO_DAYS(NOW()) <= '.$bc['family_limit'];
            }elseif ($asid==5) {
                $where['market.status_id'] = 2;
                $where['_string'] = 'market.end_time!="" and ABS(TO_DAYS(from_unixtime(market.end_time)) - TO_DAYS(NOW())) < '.$bc['over_limit'];
            } else {
                $where['market.status_id'] = $asid;
            }
            $params[] = "asid=".$asid;
        }

        if (isset($_GET['ssids'])) {
            $this->ssids = trim($_GET['ssids']);
            $where['market.settle_state'] = array("in", $this->ssids);
            $params[] = "ssids=" . $this->ssids;
        }

        if (isset($_GET['ssid'])) {
            $this->ssids = $ssid = trim($_GET['ssid']);
            $where['market.settle_state'] = $ssid;
            $params[] = "ssid=".$ssid;
        }

        if ($_GET['act'] != "search" && $_GET['category_id']) {
        ...
        $this->category_list = M('product_category')->field("category_id, name")->cache(true)->where(array("enable="=>1, "league_id"=>session('league_id')))->order("order_id asc")->select();

        $market_status = array(
            "as"=>array('运行状态', '1,2,3,4,5'),
            "ss"=>array('结算状态', '913,914,915,916,919,917,918')
        );
        foreach($market_status as $k=>$v) {
            $status_where = array("model"=>$k,"deformity"=>0);
            $status_where['status_id'] = array("in", $v[1]);
            $sas = array(
                "title"=>$v[0],
                'sid'=>$v[1],
                "slist"=>M('market_status')->field("name,status_id, model")->cache(true)->where($status_where)->order("order_id asc")->select()
            );;
            if ($asid && in_array($asid, explode(",", $v[1]))) {
                $sas['csid'] = M('market_status')->cache(true)->where("status_id=".$asid)->find();
            }

            if ($ssid && in_array($ssid, explode(",", $v[1]))) {
                $sas['csid'] = M('market_status')->field("name,status_id, model")->cache(true)->where("status_id=".$ssid)->find();
            }
            $status_list[$k] = $sas;
        }
        $this->status_list = $status_list;
        $this->show_list_index_html($where, $params, "订单表");

    }

    public function display_index_html() {
        $field_array = array();
        $field_array_show = $this->format_index_fields(getIndexFields(array("market"), null, "market_list_header_fields"));
        foreach($field_array_show as $k=>$v) {
            if ($v['viewid'] != "" && $v['viewid'] != "255") {
                if (!in_array($this->assort, explode(" ",$v['viewid']))) {
                    continue;
                }
            }
            $field_array[] = $v;
        ...
        $this->field_list =getMainFields("market", null, "market_list_search_fields");
        $this->alert = parseAlert();
    ...
    public function make_list_order(&$params) {
        ...
        if($_GET['desc_order']){
            if ($_GET['desc_order'] == "agency_scale") {
                $order = 'product_agency_scale desc';
            } else {
                $order = trim($_GET['desc_order']).' desc';
            }
            $params[] = "desc_order=" . trim($_GET['desc_order']);
        }elseif($_GET['asc_order']){
            if ($_GET['asc_order'] == "agency_scale") {
                $order = 'product_agency_scale asc';
            } else {
    ...
    public static function replenish_market_list($list) {
        foreach($list as $key => $value){
            $list[$key] = self::replenish_list_item($value);
            $list[$key]['settle_status_name'] = format_market_status($value['settle_state'], $value['is_cancel_submit']);
            if (!$list[$key]['branch_id']) {
                $list[$key]['branch']['name'] = "公司总部";
            }
            if (session('?admin')) {
            ...
            $list[$key]['service_status_id'] = format_market_status($list[$key]['service_status_id'], $list[$key]['is_cancel_submit']);
        }
        return $list;
    }
    ...
    public function replenish_list($list, $export) {
        ...
        return self::replenish_market_list($list);
    }
    ...
    public function listDialog(){
        ...
        switch($_GET['module']) {
            case "product": {
                $model = "MarketProductView";
                break;
            }
            case "channel": {
                $model = "MarketChannelView";
                break;
            }
            case "urge": {
                $model = "MarketUrgeView";
                break;
            }
            default: {
                $model = "MarketView";
        ...
        $where['league_id'] = session('league_id');
        $this->ajaxReturn(make_data_list($model, $where, $data_field, array($this, "format_dialog_item")),'JSON');
    }
    ...
    public function parse_dialog_where() {
        ...
        if ($_GET['module'] && $_GET['id']) {
            $m = trim($_GET['module']);
            if ($m == "channel") {
                $where["channel_model"]=trim($_GET['channel_model']);
                $where["channel_model_id"]=trim($_GET['id']);
            }elseif ($m == "urge"){
                $where["staff_id"]=trim($_GET['id']);
            } else {
                $where[$m.".".$m."_id"]=$_GET['id'];
            }
        }
        $where['league_id'] = session('league_id');
        return $where;
    ...
    public function format_dialog_item($val) {
        $val["market_id"] = array(
            ...
            "settle_state"=>$val['settle_state'],
        );
        $val["settle_status_name"] = format_market_status($val['settle_state'], 0);
        return $val;
    }

    public function product_listDialog(){
        if ($this->isAjax() === false) {
            role_log(MODULE_NAME, ACTION_NAME, $_REQUEST);
            return $this->display();
        }
        ...
        $data_field = array(
            ...
            array(
                "field"=>"real_start_time_show",
                "order"=>"real_start_time"
            ),
            array(
                "field"=>"real_end_time_show",
                "order"=>"real_end_time"
            ),
        );
        $where['_string'] =trim($_GET['query']);
        $where["league_id"] = session("league_id");
        $this->ajaxReturn(make_data_list("MarketProductView", $where, $data_field, array($this, "format_product_dialog_item")),'JSON');
    }

    public function format_product_dialog_item($val) {
        $val["product_id"] = array(
            "product_id"=>$val['product_id'],
            "market_product_id"=>$val['market_product_id']
        );
        $val["real_end_time_show"] = toDate($val["real_end_time"], 'Y-m-d');
        $val["real_start_time_show"] = toDate($val["real_start_time"], 'Y-m-d');
        return $val;
    }
    ...
    public function update_default_urge_price($market_ids) {
        ...
        foreach($markets as $market) {
            ...
            $branch_id = $market["branch_id"];      //总店使用大望路系数
            if ($branch_id) {
                $branch = M("branch")->where("branch_id=".$branch_id)->find();
            } else {
                $branch = M("branch")->where(array("league_id"=>session('league_id')))->order("branch_id asc")->find();
            }
            $urge_branch_ratio = 0;
