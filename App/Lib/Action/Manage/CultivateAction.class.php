...
<?php 
class CultivateAction extends BaseAction {
	public function _initialize(){
        ...
        $action = array(
			...
			'allow'=>array(
                ...
                'changecontent',
                'cultivatelock',
                'check_cultivate_limit',
                'excelimportdownload',
                'search',
                'getcultivateoriginal',
                "cancel_submit",
                "mission",
                "payment_status"
            )
        ...
        if (ACTION_NAME == "index") {
            if (($_REQUEST['act'])) {
                if ($_REQUEST['act'] == "advsearch") {
                    ...
                    if (!vali_permission("cultivate", ACTION_NAME, $_REQUEST['act'])) {
                        if(isAjaxRequest()){
            ...
            if ($_REQUEST['assort']) {
                if (vali_permission("cultivate", ACTION_NAME, $_REQUEST['assort'])) {
        ...
        else if (ACTION_NAME == "submit_settlement") {
            if (vali_permission("cultivate", "edit/basic")) {
                $action['permission'][] = "submit_settlement";
            }
        }
    ...
    public function show_list($where = array(), $params = array()) {
        ...
        if ($_GET['lia'] == 'self') {
            ...
            if (!session('?admin') && vali_permission("branchlock", "cultivate") && session('restriction') === true) {
        ...
        if ($branch_id != "") {
            $where['cultivate.branch_id'] = $branch_id;
        ...
        if (isset($_GET['asids'])) {
            ...
            $where['cultivate.status_id'] = array("in", $this->asids);
            $params[] = "asids=" . $this->asids;
        } else {
            $where['cultivate.status_id'] = array("neq", '0');
        }
        ...
        if (isset($_GET['asid'])) {
            ...
            if ($asid == 4) {
                $where['cultivate.status_id'] = 1;
                $where['_string'] = 'cultivate.start_time!="" and TO_DAYS(from_unixtime(cultivate.start_time)) - TO_DAYS(NOW()) <= '.$bc['family_limit'];
            }elseif ($asid==5) {
                $where['cultivate.status_id'] = 2;
                $where['_string'] = 'cultivate.end_time!="" and ABS(TO_DAYS(from_unixtime(cultivate.end_time)) - TO_DAYS(NOW())) < '.$bc['over_limit'];
            } else {
                $where['cultivate.status_id'] = $asid;
            }
            $params[] = "asid=".$asid;
        }
        ...
        if (isset($_GET['ssids'])) {
            ...
            $where['cultivate.settle_state'] = array("in", $this->ssids);
            $params[] = "ssids=" . $this->ssids;
        }

        if (isset($_GET['ssid'])) {
            $this->ssids = $ssid = trim($_GET['ssid']);
            $where['cultivate.settle_state'] = $ssid;
            $params[] = "ssid=".$ssid;
        }

        if (isset($_GET['csids'])) {
            $this->csids = trim($_GET['csids']);
            $where['cultivate.cert_state'] = array("in", $this->csids);
            $params[] = "csids=" . $this->csids;
        }

        if (isset($_GET['csid'])) {
            $this->csids = $csid = trim($_GET['csid']);
            $where['cultivate.cert_state'] = $csid;
            $params[] = "csid=".$csid;
        }

        if (isset($_GET['esids'])) {
            $this->esids = trim($_GET['esids']);
            $where['cultivate.examine_state'] = array("in", $this->esids);
            $params[] = "esids=" . $this->esids;
        }

        if (isset($_GET['esid'])) {
            $this->esids = $esid = trim($_GET['esid']);
            $where['cultivate.examine_state'] = $esid;
            $params[] = "esid=".$esid;
        }
        ...
        $cultivate_status = array(
            ...
            "cs"=>array('证书状态', '20,21,22,23,24'),
            "es"=>array('考试状态', '12,11,10'),
        );
        foreach($cultivate_status as $k=>$v) {
            $status_where = array("model"=>$k,"deformity"=>0);
            $status_where['status_id'] = array("in", $v[1]);
            $sas = array(
                "title"=>$v[0],
                'sid'=>$v[1],
                "slist"=>M('CultivateStatus')->cache(true)->where($status_where)->order("order_id asc")->select()
            );;
            if ($asid && in_array($asid, explode(",", $v[1]))) {
                $sas['csid'] = M('CultivateStatus')->cache(true)->where("status_id=".$asid)->find();
            }
            if ($ssid && in_array($ssid, explode(",", $v[1]))) {
                $sas['csid'] = M('CultivateStatus')->cache(true)->where("status_id=".$ssid)->find();
            }
            if ($csid && in_array($csid, explode(",", $v[1]))) {
                $sas['csid'] = M('CultivateStatus')->cache(true)->where("status_id=".$csid)->find();
            }
            if ($esid && in_array($esid, explode(",", $v[1]))) {
                $sas['csid'] = M('CultivateStatus')->cache(true)->where("status_id=".$esid)->find();
            }
            $status_list[$k] = $sas;
        }
        $this->status_list = $status_list;
        self::show_list_index_html($where, $params, "培训订单表");
    }
    ...
    public function display_index_html() {
        $field_array = array();
        $field_array_show = $this->format_index_fields(getIndexFields(array("cultivate")));
        ...
        $this->field_list = getMainFields("cultivate");
    ...
    public function replenish_list($list, $export) {
        ...
        return self::replenish_cultivate_list($list);
    }
    ...
    function perfect_list_item($value, $export = false, $branchlock = false) {
        $m_model = M($value['model'])->where(array($value['model']."_id"=>$value['model_id']))->find();;
        $value["model_idcode"] = $m_model?$m_model['idcode']:"";
        $value["model_name"] = $m_model?$m_model['name']:"";
    ...
    public static function replenish_cultivate_list($list) {
        foreach($list as $key => $value){

            $list[$key] = self::replenish_list_item($value);
            $list[$key]['settle_status_name'] = format_cultivate_status($value['settle_state'], $value['is_cancel_submit']);
            ...
            $list[$key]['status_name'] = format_cultivate_status($list[$key]['status_id'], $list[$key]['is_cancel_submit']);
        }
        return $list;
    }
    ...
    public function listDialog(){
        ...
        switch($_GET['module']) {
            case "channel": {
                $model = "CultivateChannelView";
                break;
            }
            case "urge": {
                $model = "CultivateUrgeView";
                break;
            }
            default: {
                $model = "CultivateView";
    ...
    public function format_dialog_item($val) {
        $val["cultivate_id"] = array(
            "cultivate_id"=>$val['cultivate_id'],
            "settle_state"=>$val['settle_state'],
        );
        $m_model = M($val['model'])->where(array($val['model']."_id"=>$val['model_id']))->find();;
        $val["model_idcode"] = $m_model?$m_model['idcode']:"";
        $val["model_name"] = $m_model?$m_model['name']:"";
