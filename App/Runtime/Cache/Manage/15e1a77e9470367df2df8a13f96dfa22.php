...
<div class="container">
    <div class="row-fluid">
        <div class="span12">
            <div></div>
            <table class="table table-hover" id="customer_dialog_table">
                <thead>
                <tr>
...
<script id="customer-checkbox" type="text/x-jquery-tmpl">
<input type="radio" name="customer" value="${customer_id}" />
</script>

<script>
    var reqparam = art.dialog.data('reqparam');
    var urlajax = {
        "url": "<?php echo U('Customer/listDialog');?>"
    };
    if (reqparam) {
        urlajax['data'] = reqparam;
    }
    $('#customer_dialog_table').dataTable({
        ajax:urlajax,
        "columnDefs": [
            {
                "bSortable": false,
                ...
                "createdCell": function (td, cellData, rowData, row, col) {
                    $(td).html($('#customer-checkbox').tmpl(cellData));
                }
            }
        ],
        "pageLength": 7,
