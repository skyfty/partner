...
<div ng-controller="marketAssociateController">
    ...
    <div class="row wrapper">
        <div class="table-responsive">
            <table ui-datatable="fields" ng-data-url="market/index" ng-page="totalPages" ng-create-row="createdRow" ng-row-option="table-row-options" ng-page-length="datatablePageLength" class="table table-striped hover b-t b-light" style="white-space: nowrap;">
                <thead>
                <tr>
                </tr>
                </thead>
...
<div class="btn-toolbar btn-toolbar-light" id="user-toolbar-options">
    <a target="_blank" href="#/view/market/${market_id}"><i class="icon-user"></i></a
</div>
</script>


<script id="table-row-selected-hot" type="text/x-jquery-tmpl">
<a id="table-cell-option" target="_blank"  href="#/view/market/${market_id}" class="table-cell-option" style="margin-left: 5px"><i class="fa fa-flickr"></i></a>
</script>

<script>
    var fields = <?php echo (json_encode($field_array)); ?>;

    app.controller('marketAssociateController', function($scope, multipleSearch) {
        $scope.fields = fields;
        ...
        $scope.datatablesearch = function(d){
            ...
            d['dataview'] = upperFirst($scope.related_model) + "MarketView";
            d['forces'] = {
                'market_product.product_id':$scope.related_model_id
            };
        };
    });
</script>