...
function U2($url='',$vars='',$suffix=true,$redirect=false,$domain=true) {
    ...
    if($domain) {
        $domain = $_SERVER['HTTP_HOST'];
        $pos = stripos($url, "index.php");
        if ($pos == 1) {
            $url = substr_replace($url, "", 1, 9);
        }
        $pos = stripos($url, "_m_");
        if ($pos) {
            $url = substr_replace($url, "", $pos, 3);
        }
        $pos = stripos($url, "_a_");
        if ($pos) {
            $url = substr_replace($url, "_", $pos, 3);
        }
        $url   =  (is_ssl()?'https://':'http://').$domain.$url;
    }
...
function session($name,$value='') {
    ...
    if(is_array($name)) { // session初始化 在session_start 之前调用
        ...
        if(isset($name['cookie_path']))     ini_set('session.cookie_path', $name['cookie_path']);
