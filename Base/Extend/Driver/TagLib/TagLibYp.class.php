...
class TagLibYp extends TagLib
{
	protected $tags   =  array(
		...
		'subcat'=>array('attr'=>'catid,type,self,key,id,pos','level'=>3),
	...
	public function _flash($attr,$content) {
		...
		$Tpl = TMPL_PATH.GROUP_NAME.'/'.THEME_NAME.'/Slide_'.$flash['tpl'].'.html';
	...
	public function _kefu($attr,$content) {
		...
		foreach($data as $key =>$r){
            $datas.='<h3 class="titZx">'.$r['name'].'</h3><ul>';
			...
			if($r['type']==1){
				...
				foreach((array)$codes as $code){
					if($code){
						...
						$datas.='<li><a href="http://wpa.qq.com/msgrd?v=3&uin='.$code.'&site=qq&menu=yes" rel="nofollow" target="_blank"><img border="0" SRC="http://wpa.qq.com/pa?p=1:'.$code.':'.$skin.'" alt="'.$r['name'].'">'.$codename.'</a>';
					}
				}
			}elseif($r['type']==2){
				...
				foreach((array)$codes as $code){
					if($code){
						$codearr=explode("|",$code);
						$code= $codearr[0];$codename= $codearr[1];
						$datas.='<li><a href="msnim:chat?contact='.$code.'"><img src="'.__ROOT__.'/Public/img/kefu/msn'.$skin.'.gif">'.$codename.'</a></li>';
					}
				}
			}elseif($r['type']==3){
