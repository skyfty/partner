...
class ViewModel extends Model {
    ...
    protected function checkFields($fields='') {
        $fieldsreal = $fields;
        if(empty($fields) || '*'==$fields || strpos($fieldsreal, "ALL_VIEW_FIELD") !== false) {
            // 获取全部视图字段
            $fields =   array();
            foreach ($this->viewFields as $name=>$val){
                $k = isset($val['_as'])?$val['_as']:$name;
            ...
            if (strpos($fieldsreal, "ALL_VIEW_FIELD") !== false) {
                $fields = str_replace("ALL_VIEW_FIELD", $fields, $fieldsreal);
            }
        }else{
            if(!is_array($fields))
